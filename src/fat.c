...
static int fatEndCluster(FAT* fat, size_t cluster);
...
static void fatDirSectorSearch(FAT* fat,
    ...
    DIRENT** lfn_start_de,
    size_t* lfn_start_cluster,
    size_t* lfn_start_dirSect);
static void fatReadLogical(FAT* fat, size_t sector, void* buf);
static void fatWriteLogical(FAT* fat, size_t sector, void* buf);
static void fatMarkCluster(FAT* fat, size_t cluster);
static size_t fatFindFreeCluster(FAT* fat);
static void fatChain(FAT* fat, FATFILE* fatfile);
static void fatNuke(FAT* fat, size_t cluster);
...
int fatInit(FAT* fat,
    unsigned char* bpb,
    void (*readLogical)(void* diskptr, size_t sector,
        void* buf),
    void (*writeLogical)(void* diskptr, size_t sector,
        void* buf),
    void* parm,
...
static void checkFSInfoSignature(FAT* fat)
{
    ...
    if (buf[0] == 0x52 && buf[1] == 0x52 && buf[2] == 0x61 && buf[3] == 0x41 &&
        ...
        buf[484] == 0x72 && buf[485] == 0x72 && buf[486] == 0x41 && buf[487] == 0x61 &&
    ...
    int attrib)
{
    ...
    if (fat->pos_result == FATPOS_FOUND) {
        ...
        fat->currcluster = p->start_cluster[1] << 8
            | p->start_cluster[0];
        if (fat->fat_type == 32) {
            fat->currcluster = fat->currcluster | (((size_t)p->access_rights[0] << 16) | ((size_t)p->access_rights[1] << 24));
        }
    ...
    if (found || fat->pos_result == FATPOS_ONEMPTY) {
        ...
        fatPopulateDateTime(fat, p, (FATDATETIME_UPDATE_MODIFY | FATDATETIME_UPDATE_CREATE | FATDATETIME_UPDATE_ACCESS));
    ...
    int attrib)
{
    ...
    if (fat->pos_result == FATPOS_ONEMPTY) {
        ...
        memset(dot.file_name, ' ', sizeof(dot.file_name) + sizeof(dot.file_ext));
...
unsigned int fatOpenFile(FAT* fat, const char* fnm, FATFILE* fatfile)
{
    ...
    if (strcmp(fnm, "") == 0) {
        ...
        fatfile->fileSize = p->file_size[0]
            | ((size_t)p->file_size[1] << 8)
            | ((size_t)p->file_size[2] << 16)
            | ((size_t)p->file_size[3] << 24);
        fatfile->currpos = 0;
        ...
        fatfile->lastBytes = (unsigned int)(fatfile->fileSize
            % ((size_t)fat->sectors_per_cluster
    ...
    if (!((fatfile->root) && (fat->rootsize))) {
        ...
        if (fatfile->nextCluster == 0) {
            ...
            /* sectorUpto is potentially wrong after a seek/write too,
               so we recalculate */
            fatfile->sectorUpto = (fatfile->currpos % (fat->sectors_per_cluster * MAXSECTSZ))
                / MAXSECTSZ;
        }
    }
    ...
    unsigned int szbuf, unsigned int* writtenbytes)
{
    ...
    if ((fatfile->currpos < fatfile->fileSize) || ((fatfile->lastBytes != 0) && (rem != 0))) {
        fatReadLogical(fat,
            fatfile->sectorStart + fatfile->sectorUpto,
            bbuf);
    }
    ...
    fatPopulateDateTime(fat, d, (FATDATETIME_UPDATE_MODIFY | FATDATETIME_UPDATE_ACCESS));
...
static void fatUpdateLastAccess(FAT* fat, FATFILE* fatfile)
{
    ...
    if (fatfile->last_access == access_date)
        return;
...
long fatSeek(FAT* fat, FATFILE* fatfile, long offset, int whence)
{
    ...
    if (whence == SEEK_SET)
        ...
        newpos = fatfile->currpos + offset;
    else if (whence == SEEK_END)
        newpos = fatfile->fileSize + offset;
    ...
    fatfile->sectorUpto = ((realpos - adjust) % (fat->sectors_per_cluster * MAXSECTSZ))
        / MAXSECTSZ;
...
static void fatPosition(FAT* fat, const char* fnm)
{
    ...
    if (fat->notfound) {
        ...
        if (fat->pos_result == FATPOS_ONEMPTY || fat->pos_result == FATPOS_ENDCLUSTER)
            fat->c_path[0] = '\0';
...
static void fatReadLogicalCached(FAT* fat, size_t sector, void* buf)
{
    ...
    size_t* nextCluster)
{
    ...
    *startSector = (cluster - FIRST_DATA_CLUSTER)
    ...
    if (fat->fat_type == 16) {
        ...
        *nextCluster = (buf[offset]
                           | ((size_t)buf[offset + 1] << 8)
                           | ((size_t)buf[offset + 2] << 16)
                           | ((size_t)buf[offset + 3] << 24))
            & 0x0fffffff;
    }
    return;
}
                    ...
                    if (*lfn_len && generateChecksum(p->file_name) == *checksum) {
                        ...
                        if (!fat->last) {
                            ...
                            for (i = 0; i < fat->path_part_len; i++) {
                                fat->c_path[i] = toupper((unsigned char)(fat->upto - 1
                                    - fat->path_part_len)[i]);
                            }
                        } else {
                            ...
                            for (i = 0; i < fat->path_part_len; i++) {
                                fat->c_path[i] = toupper((unsigned char)((fat->upto
                                    - fat->path_part_len)[i]));
                            }
                        }
    ...
    if (fat->fat_type == 16) {
        ...
        for (;
            ...
            for (; x < MAXSECTSZ; x += 4) {
                ...
                if ((buf[x] == 0) && (buf[x + 1] == 0) && (buf[x + 2] == 0) && ((buf[x + 3] & 0xf) == 0)) {
                    found = 1;
                    break;
                }
            }
...
static void fatChain(FAT* fat, FATFILE* fatfile)
{
    ...
    if (fatEndCluster(fat, oldcluster)) {
        printf("internal error - attempt to chain %lu to %lu\n",
            (unsigned long)oldcluster, (unsigned long)newcluster);
        PosMonitor();
    }
    if (fat->fat_type == 16) {
        ...
        if (oldcluster % 2)
            buf[offset + 1] = newcluster >> 4;
        else
            buf[offset + 1] = (buf[offset + 1] & 0xf0) | (newcluster >> 8);
...
unsigned int fatRenameFile(FAT* fat, const char* old, const char* new)
{
    ...
    /* Copies LFN from new. */
        ...
        memcpy(fat->new_file, new, lfn_len);
    fatPosition(fat, old);
    if (fat->notfound)
        return (POS_ERR_FILE_NOT_FOUND);
...
static unsigned int isLFN(const char* fnm, unsigned int length)
{
    ...
    for (i = 0; i < length; i++) {
        ...
        if (fnm[i] == '+' || fnm[i] == ',' || fnm[i] == '.' || fnm[i] == ';' || fnm[i] == '=' || fnm[i] == '[' || fnm[i] == ']')
            return (length);
    }
    ...
    unsigned int* length)
{
    ...
    if (p->file_name[0] & 0x40) {
        ...
        for (i = 0; i < 13; i++) {
             ...
             * the end of the LFN. */
            if (p->file_name[i * 2 + offset] == 0x00 && p->file_name[i * 2 + offset + 1] == 0x00)
                break;
            ...
            for (p = (DIRENT*)buf;
                ...
                for (i = 0; i < 13; i++) {
                    ...
                    if ((required_free - 1) * 13 + i < lfn_len) {
                        ...
                        p->file_name[i * 2 + offset] = orig_lfn[(required_free - 1) * 13 + i];
    ...
    unsigned char* shortname)
{
    ...
    for (i = lfn_len - offset - 1; i > 0; i--) {
        if ((lfn + offset)[i] == '.') {
            ...
            for (j = 0; j < 3 && j + i + 1 < lfn_len - offset; j++) {
                ...
                if (up_char > 0x7F || up_char == '+' || up_char == ',' || up_char == ';' || up_char == '=' || up_char == '[' || up_char == ']')
                    shortname[8 + j] = '_';
                else
                    shortname[8 + j] = (char)up_char;
            }
    ...
    for (i = 0; j < 6 && i < dot_pos && i < lfn_len; i++) {
        if ((lfn + offset)[i] == ' ' || (lfn + offset)[i] == '.')
            continue;
        up_char = toupper((lfn + offset)[i]);
        if (up_char > 0x7F || up_char == '+' || up_char == ',' || up_char == ';' || up_char == '=' || up_char == '[' || up_char == ']')
            shortname[j] = '_';
        else
            shortname[j] = (char)up_char;
        j++;
    }
...
{
    DIRENT* stored_de;
    size_t stored_currcluster;
    size_t stored_dirSect;
    /* +++Add UCS-2 support. */
    ...
    while (1) {
        ...
        eod = 0;
        /* Loops over clusters. */
        while (!fatEndCluster(fat, fat->currcluster) || (fat->processing_root && fat->rootsize)) {
            ...
            for (x = 0; x < numsectors; x++) {
                ...
                for (p = (DIRENT*)buf;
                    ...
                    if (p->file_name[0] == DIRENT_DEL || p->file_attr == DIRENT_LFN) {
    ...
    while (1) {
        ...
        for (x = 0; x < *numsectors; x++) {
            ...
            for (p = (DIRENT*)buf;
                ...
                /* Checks if the numeric tail is unique
                 * if the generated name has numeric tail. */
                if (fat->lossy_conversion && p->file_attr != DIRENT_LFN && memcmp(p->file_name, shortname, 11) == 0) {
                    ret = incrementNumericTail(shortname);
                    if (ret)
                        return (ret);
                }
